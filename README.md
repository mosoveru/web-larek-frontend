...
На какие события реагирует класс ``EventEmitter`` описаны в разделе ``Ключевые типы данных``
...
Каждый экземпляр класса ``CommonUI<T>`` отвечает за свою область отображения, благодаря разным функциям-рендерам и шаблонам. Примеры событий, на которые реагирует класс, описан ниже, механизм работы отображения при разных событиях описан в разделе ``Компоненты модели данных (бизнес-логика)``.  
Поля класса:  
